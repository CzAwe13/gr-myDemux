...
#include "my_header_payload_demux_impl.h"
...
const pmt::pmt_t my_header_payload_demux_impl::msg_port_id()
{
...
my_header_payload_demux::sptr
my_header_payload_demux::make(int header_len,
...
{
    return gnuradio::make_block_sptr<my_header_payload_demux_impl>(header_len,
                                                                items_per_symbol,
                                                                guard_interval,
                                                                length_tag_key,
                                                                trigger_tag_key,
...
my_header_payload_demux_impl::my_header_payload_demux_impl(
    int header_len,
    int items_per_symbol,
    int guard_interval,
    const std::string& length_tag_key,
...
void my_header_payload_demux_impl::forecast(int noutput_items,
...
bool my_header_payload_demux_impl::check_buffers_ready(int output_symbols_reqd,
...
int my_header_payload_demux_impl::general_work(int noutput_items,
                                            gr_vector_int& ninput_items,
...
int my_header_payload_demux_impl::find_trigger_signal(int skip_items,
                                                   int max_rel_offset,
...
void my_header_payload_demux_impl::parse_header_data_msg(pmt::pmt_t header_data)
{
...
void my_header_payload_demux_impl::copy_n_symbols(const unsigned char* in,
                                               unsigned char* out,
                                               int port,
...
void my_header_payload_demux_impl::update_special_tags(uint64_t range_start,
                                                    uint64_t range_end)
{
    if (d_track_time) {
        std::vector<tag_t> tags;
...
void my_header_payload_demux_impl::add_special_tags()
{
    if (d_track_time) {
